...
'03 · 敵対化の回避：不用意に派閥の検問所を攻撃すると評判が大幅ダウン。-300以下で即座に銃撃されるため注意。',
  ...
  },
  {
    ...
    answer: 'Scavlandの地下バンカー（Subterranean Bunkers）は、ザレシエ（Zalesye）荒野において最高難易度かつ最高峰の軍用物資を獲得できる重要攻略拠点です。最大の複合施設「セクターB-4（Sector B-4）」は北西部の森林地帯に位置し、コンクリートの排水トレンチと重厚な防爆ハッチが目印となります。内部の軍用金庫室を開けるには、耐久度3回の「赤キーカード（Red Keycard）」が必要です。バンカー内の物資は退出後3時間（またはセーフハウスでの睡眠によるゲーム内24時間周期）で再配置されます。警報サイレン作動時は背面の非常用換気シャフトから脱出しましょう。',
    steps: [
      '01 · 北西部森林のB-4バンカー入口を特定：ザレシエ北西の森に入り、草木に覆われた線路に沿ってコンクリート排水トレンチを進むと、哨戒兵と放射性水たまりの奥に重防爆ハッチがあります。',
      '02 · 赤キーカード（Red Keycard）の入手：検問所の指揮官ドロップ、高危険度エリアのエアードロップ、またはナージャの討伐契約報酬から獲得します。消耗品ですが3回の解錠耐久度を持っています。',
      '03 · 周囲の安全確保とキーカード認証：地上の敵を排除してから階段を下り、緑色に光る端末に赤キーカードを通します。開扉中は約15秒間のサイレンが鳴り響き、周囲の敵を呼び寄せるため迎撃の準備をしてください。',
      '04 · 狭隘通路の制圧と軍用コンテナ回収：狭いコンクリート通路を進み、至近距離戦用ショットガンや高貫通ライフルで変異体を排除。軍用チェストから高位アタッチメントや弾薬、軍用プレートを回収します。',
      '05 · バンカーのリセット周期を把握：Update 0.6.0より、地下バンカーは退出後3時間経過（またはセーフハウスのベッドで寝てゲーム内24時間周期を経過）することでコンテナや物資が再配置されます。',
      '06 · 非常用換気シャフトからの緊急脱出：正面入口はサイレンで敵が集まりやすいため、探索後はバンカー奥の非常用換気シャフトをよじ登って地上の森林へ安全に脱出するのが鉄則です。'
    ],
    facts: [
    ...
    faq: [
      ['地下バンカー（B-4）はマップのどこにありますか？', 'ザレシエ北西の森林地帯にあります。草木に覆われた古い線路の先にあるコンクリート排水トレンチと、頑丈な防爆扉を探してください。'],
