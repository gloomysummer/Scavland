...
export const ruGuides: Guide[] = [
  ...
  },
  {
    ...
    faq: [
      ['Вырастет ли цена после выхода из раннего доступа?', 'NoShadow не объявляли новую цену для полной версии. Пока игра находится в раннем доступе, действует текущая цена в Steam; об изменениях разработчики сообщают в новостях Steam.'],
  ...
  },
  {
    ...
    answer: 'Scavland выходит с полной поддержкой контроллеров и, по словам разработчиков, «задуман с учётом Steam Deck и портативной игры», поэтому отдельная перенастройка не требуется. В обновлении 0.7.0 также указано «улучшение масштабирования текста на Steam Deck». Настройки ниже — это отправные точки, а не результаты замеров: начните со стандартного профиля игры и постепенно снижайте лимит кадров или TDP, если хотите более тихую и долгую сессию.',
    steps: [
      'Proton: поведение зависит от системы. Если появятся проблемы с шейдерами или звуком, попробуйте сменить совместимость в свойствах игры в Steam.',
      'Графика: начните с нативного для Steam Deck разрешения 1280x800 в полноэкранном режиме и подберите тени и сглаживание под свой вкус.',
      'Питание (Quick Access): в меню быстрого доступа SteamOS можно ограничить частоту кадров и TDP — снижайте лимиты постепенно, следя за счётчиком кадров.',
      'Управление: правый трекпад можно назначить мышью для прицеливания; готовые раскладки сообщества доступны в настройках контроллера Steam.'
    ],
    facts: [
